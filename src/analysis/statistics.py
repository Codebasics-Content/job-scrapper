...
from src.analysis.analysis.skill_calculator import calculate_skill_percentage
from src.analysis.analysis.skill_extractor import extract_all_skills
...
def get_top_skills(jobs: list[JobModel], top_n: int = 20) -> list[tuple[str, float]]:
    ...
    skill_percentages: list[tuple[str, float]] = []
...
def generate_skill_report(jobs: list[JobModel], min_percentage: float = 1.0) -> Mapping[str, str | int | dict[str, int] | dict[str, dict[str, int | float]]]:
    """Generate comprehensive skill analysis report"""
    if not jobs:
        return {"error": "No jobs provided for analysis"}
    
    all_skills = extract_all_skills(jobs)
    skill_analysis: dict[str, dict[str, int | float]] = {}
