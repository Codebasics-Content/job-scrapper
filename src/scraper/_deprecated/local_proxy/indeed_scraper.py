"""Indeed scraper using LOCAL BrightData Proxy Manager + Playwright.

Fast method:
- Local proxy server (luminati-proxy)
- BrightData's residential IPs (US)
...
async def scrape_indeed_jobs_local_proxy(
    query: str,
    location: str = "United States",
    limit: int = 50
) -> List[JobModel]:
    """Scrape Indeed using local BrightData proxy + Playwright.
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # Extract job cards
            job_cards = await page.query_selector_all(".job_seen_beacon, .jobsearch-SerpJobCard, .tapItem")
            print(f"   Found {len(job_cards)} job cards")
            
            for idx, card in enumerate(job_cards[:limit], 1):
                try:
                    # Extract job link
                    link_elem = await card.query_selector("a.jcs-JobTitle, h2.jobTitle a")
                    if not link_elem:
                        continue
                    ...
                    job_id = f"indeed_{job_id_match.group(1)}" if job_id_match else job_url
                    
                    # Extract title
                    title_elem = await card.query_selector("h2.jobTitle span, .jobTitle")
                    ...
                    salary_elem = await card.query_selector(".salary-snippet, .metadata.salary-snippet-container")
...
async def _fetch_job_description(page: Page, job_url: str) -> str:
    ...
    try:
        ...
        desc_elem = await page.query_selector("#jobDescriptionText, .jobsearch-jobDescriptionText")
...
async def test_scraper():
    ...
    jobs = await scrape_indeed_jobs_local_proxy(
